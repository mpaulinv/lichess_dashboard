...
def display_data_summary(csv_dict):
    ...
    with st.expander("🔍 Debug Data Structure", expanded=False):
        st.text("Data structure contents:")
        for key, data in csv_dict.items():
            df = get_dataframe_from_result(csv_dict, key)
            if df is not None:
                st.text(f"{key}: {len(df)} rows, columns: {list(df.columns)}")
            else:
                st.text(f"{key}: No valid data")
...
def create_opening_charts(csv_dict):
    ...
    try:
        ...
        # Debug: Show what columns we have (remove in production)
        with st.expander("🔍 Opening Data Debug", expanded=False):
            st.write(f"Opening data columns: {list(opening_df.columns)}")
            st.write(f"Opening data shape: {opening_df.shape}")
            st.write("Sample data:")
            st.dataframe(opening_df.head())
        ...
        if all(col in opening_df.columns for col in ['UserColor', 'TotalGames', 'Win', 'Draw', 'Loss']):
            white_perf = opening_df[opening_df["UserColor"] == "white"].nlargest(8, "TotalGames")
            if len(white_perf) > 0:
                ...
                fig_white_perf = px.bar(
                    ...
                    y="Percentage",
                    color="Result",
                    title="Top 8 Openings Performance as White (%)",
        ...
        if all(col in opening_df.columns for col in ['UserColor', 'TotalGames', 'Win', 'Draw', 'Loss']):
            black_perf = opening_df[opening_df["UserColor"] == "black"].nlargest(8, "TotalGames")
            if len(black_perf) > 0:
                # Reshape data for stacked bar chart
                ...
                fig_black_perf = px.bar(
                    ...
                    y="Percentage",
                    color="Result",
                    title="Top 8 Openings Performance as Black (%)",
                    labels={"Percentage": "Percentage (%)", "OpeningName": "Opening"},
                    text="Percentage",
                    color_discrete_map={
                        "Win": "#2E8B57",
...
def main():
    ...
    if st.sidebar.button("🚀 Load Data", type="primary"):
        ...
        # Show loading spinner with date info
        with st.spinner(f"Fetching data for {username} from {start_date} to {end_date}..."):
            try:
    ...
    if 'csv_dict' in st.session_state and st.session_state['csv_dict']:
        ...
        with st.expander("ℹ️ How to use this dashboard", expanded=True):
            ...
            ### Tips:
            - Start with a smaller date range (1-2 weeks) for faster processing
            - Use filters to focus on specific types of games
            - Check the debug sections if you encounter issues
            """)
